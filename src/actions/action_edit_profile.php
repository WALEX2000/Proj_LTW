...
if(check_user_password($username,$current_password)){
  $new_user_info = array('username' => $username, 'name' =>$name, 'email' =>$email, 'birthday' =>$birthday, 'nationality' =>$nationality, 'password' =>$password);
  $old_user_info = get_user_info($username);
  update_user_info($old_user_info,$new_user_info);
